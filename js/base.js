...
(function () {
    ...
    function pop(arg) {
        ...
        // 判断arg是否为字符串类型，如果是，则将 arg 赋值给 conf.title
        ...
        // 获取弹框中删除操作按钮模块中‘确定’按钮
        $confirm = $content.find('button.confirm');
        // 获取弹框中删除操作按钮模块中‘取消’按钮
        $cancel = $content.find('button.cancel');

        timer = setInterval(function () { 
            // 轮询confirmed状态，confirmed用于表示用户是否进行了点击操作
            // 点击确认，confirmed状态为 true；点击取消，confirmed状态为 false；点击蒙版，confirmed状态为 false
    ...
    // 通过监听清单提醒信息中, '知道了'的按钮，点击进行隐藏清单通知信息
    ...
    function listen_task_detail() {
        ...
        $('.task-item').on('dblclick', function () {
            ...
            show_task_detail(index); // 显示清单详情
        });
        //监听单条清单点击详情细操作  $task_detail_trigger为单条清单的‘详情’按钮
    ...
    function show_task_detail(index) {
        render_task_detail(index); // 渲染指定清单详情信息
        current_index = index; 
        $task_detail.show(); // 清单详情DOM元素 显示
        $task_detail_mask.show(); // 清单详情蒙版DOM元素 显示
    }

    // 隐藏单条详情 - -隐藏详情模板区域和详情蒙版
    function hide_task_detail() {
        $task_detail.hide(); // 清单详情DOM元素 隐藏
        $task_detail_mask.hide(); // 清单详情蒙版DOM元素 
    }

    // 更新单条清单数据，并刷新localStorage数据，并重新渲染清单列表
    function update_task(index, data) {
        ...
        refresh_task_list(); // 刷新localStorage数据，并重新渲染清单列表
    }
    ...
    function render_task_detail(index) {
        ...
        var tpl = '<form>' +
            ...
            '</form>';
        $task_detail.html(null); // 将 清单详情DOM元素 清空
        $task_detail.html(tpl); // 将 指定清单DOM 添加到 清单详情DOM元素 中
        $('.datetime').datetimepicker(); // 挂载时间选择器
        // 选中模板中的form元素
        $update_form = $task_detail.find('form'); // 获取清单详情DOM元素中的form元素，$task_detail为 清单详情DOM元素
        ...
        $update_form.on('submit', function (e) {
            ...
            update_task(index, data); // 更新单条清单数据，并刷新localStorage数据，并重新渲染清单列表
    ...
    // 刷新localStorage数据，并重新渲染清单列表
    function refresh_task_list() {
        store.set('task_list', task_list);
        render_task_list(); // 渲染清单列表
    }
    ...
    function init() {
        task_list = store.get('task_list') || []; // 从缓存中加载清单数据，如果没有则将 清单列表数据数组 置为 空数组
        listen_msg_event(); // 监听清单提醒信息中，'知道了'的按钮，点击进行隐藏清单通知信息
        if (task_list.length) {
            render_task_list(); // 渲染清单列表
        }
        task_remind_check(); // 轮询清单提醒时间，如果到了提醒时间，则进行清单提醒
    }

    // 轮询清单提醒时间，如果到了提醒时间，则进行清单提醒
    ...
    function render_task_list() {
        ...
        $task_list.html(''); // 将清单列表显示区域清空
        var complete_items = []; // 用于存储已完成清单
        for (var i = 0; i < task_list.length; i++) {
            var item = task_list[i];
            // item.complete 为标识清单是否完成（已完成 - complete为true；未完成 - complete为false
            if (item && item.complete) {
                // 将已完成清单暂存储于 complete_items
                complete_items[i] = item;
            } else {
                // 将未完成清单暂存储于 $task
                var $task = render_task_item(item, i);
                // render_task_item 渲染单条清单，返回单条清单模板
            }
            $task_list.prepend($task); // 将未完成清单添加至 清单列表显示区域DOM元素 的开头
        ...
        for (var j = 0; j < complete_items.length; j++) {
            ...
            // render_task_item 渲染单条清单，返回单条清单模板
            if (!$task) continue;
            $task.addClass('completed'); // 给已完成单条清单，添加样式"completed",表示是已完成清单
            $task_list.append($task); // 将已完成清单添加至 清单列表显示区域DOM元素 的尾部
        }
        ...
        listen_task_delete(); // 监听清单删除按钮的点击事件
        listen_task_detail(); // 监听清单打开详情的点击事件
        listen_checkbox_complete(); // 监听完成任务事件 
    }

    // 渲染单条清单，返回单条清单模板，返回单条清单模板
    // data为清单数据。 index为在清单列表中的index
